...
│   │   ├── case-03-*.json    # 여름 우박
│   │   ├── case-04-*.json    # 복사 안개
│   │   ├── case-05-*.json    # 강원 영동 동풍 폭설
│   │   ├── case-06-*.json    # 열대야·도시 열섬
│   │   ├── case-07-*.json    # 장마 집중호우
│   │   ├── case-08-*.json    # 봄철 황사
│   │   ├── case-09-*.json    # 서해안 대설(해기차)
│   │   ├── case-10-*.json    # 시베리아 한파
│   │   ├── case-11-*.json    # 봄철 내륙 뇌우
│   │   ├── case-12-*.json    # 태풍 상륙
│   │   └── index.js          # 케이스 레지스트리
│   ├── components/
...
```bash
# 케이스 JSON 작성 후
node pipeline/make-demo.mjs src/cases/case-13-new.json
# src/cases/index.js에 import 추가 → cases 배열에 포함 → 자동으로 랜덤/순차 출제 풀에 포함
```
